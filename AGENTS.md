...
#### 🛡️ Guard: showScreen 화면 상태
- 불변 조건:
  - `showScreen()` 호출 후 `main-view`, `screen-study`, `screen-summary`, `screen-stats`, `screen-settings` 중 **정확히 1개**만 `display !== 'none'`이어야 한다.
  - `study` → 다른 화면 전환 시 `_studyTimerInterval`을 `clearInterval`해야 한다.
  - `stats`, `settings` 화면에서 `#bottomBtn`이 `display:none`이어야 한다.
- 테스트: `tests/showscreen-state.test.js` (`npm test`로 자동 실행)

#### 🛡️ Guard: iOS PWA CSS 보호 속성
- 불변 조건:
  - `B-57 PROTECT` 주석이 달린 CSS 선택자의 safe-area, position fixed, z-index 속성을 삭제·변경하지 않는다.
  - 보호 대상: `.study-header`, `.start-study`, `.study-content`, `.stats-header`, `.card-area`, `.stats-screen-content`, `.bottom-sheet`, `.action-sheet`, `.sync-toast`
- 테스트: `tests/css-guard.test.js` (`npm test`로 자동 실행)

#### 🛡️ Guard: CI 의존 파일 (gas/.clasp.json)
- 불변 조건:
  - `gas/.clasp.json`은 삭제·이동 금지. `.gitignore` 추가 시에도 파일 자체는 레포에 유지.
  - 정리/삭제 작업지시서 실행 전 `git status`로 삭제 예정 파일 목록 확인.
  - Deploy GAS 워크플로우는 이 파일에 의존함 (`clasp push` working-directory: gas).
